...
public:
    ...
    uint8_t GetSerializedScriptType() const {
        if (this->m_scriptType == TxoutType::PUBKEY && this->m_scriptPubKey.size() == 65) return 1;
        if (this->m_scriptType == TxoutType::PUBKEY && this->m_scriptPubKey.size() == 33) return 2;
        if (this->m_scriptType == TxoutType::PUBKEYHASH) return 3;
        if (this->m_scriptType == TxoutType::SCRIPTHASH) return 4;
        if (this->m_scriptType == TxoutType::WITNESS_V0_KEYHASH) return 5;
        if (this->m_scriptType == TxoutType::WITNESS_V0_SCRIPTHASH) return 6;
        if (this->m_scriptType == TxoutType::WITNESS_V1_TAPROOT) return 7;
        return 0;
    }

    bool IsCompressed() const {
        return this->GetSerializedScriptType() > 0;
    }
...
public:
    ...
    inline void Serialize(Stream& s) const {
        ...
        for (const auto &vin : this->m_vin) {
            std::cout << "txin: " << std::to_string(vin.GetMetadata()) << std::endl;
            controlVec.push_back(vin.GetMetadata());
            sizes.push_back(5);
        }
        for (const auto &vout : this->m_vout) {
            std::cout << "txout: " << std::to_string(vout.GetSerializedScriptType()) << std::endl;
