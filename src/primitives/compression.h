...
public:
    ...
    uint64_t GetMetadata() const {
        ...
        metadata |= this->m_isHashStandard << 1;
    ...
    inline void Unserialize(Stream& s, uint64_t& metadata, bool constructor=false) {
        ...
        m_isHashStandard = (metadata & (1 << 1)) >> 1;
        uint8_t sequenceEncoding = (metadata & (0b11 << 2)) >> 2;
...
struct CCompressedInput {
    CCompressedOutPoint outpoint;
...
public:
    ...
    inline void Unserialize(Stream& s) {
        ...
        /* Input-Output Metadata: Section 3.4-3.5 */
        std::vector<unsigned char> controlVec;
        controlVec.resize(((nInputCount * 4) + (nOutputCount * 3) + 7) / 8);
