        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (DecodeHexTx(mtx, request.params[0].get_str(), true, true)) {
        ...
        std::tuple<uint32_t, std::vector<CCompressedInput>> cinputstup = EnsureChainman(node).ActiveChainstate().CompressOutPoints(tx, warning_strings);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    try {
        ...
        std::vector<COutPoint> prevouts = EnsureChainman(node).ActiveChainstate().UncompressOutPoints(tx);
